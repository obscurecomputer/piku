package computer.obscure.piku.mod.fabric.scripting.old.components

import com.google.gson.JsonParser
import com.mojang.serialization.JsonOps
import computer.obscure.piku.mod.fabric.scripting.old.LuaNbtCompound
import computer.obscure.piku.mod.fabric.scripting.old.util.unwrap
...
interface LuaComponent {
    fun jsonToText(json: String): Component {
    ...
    fun textToJson(text: Component): String =
        ComponentSerialization.CODEC
            .encodeStart(JsonOps.INSTANCE, text)
            .getOrThrow()
            .toString()

    fun valueToLua(value: Any): Any? = when (value) {
        is Optional<*> -> {
            value.unwrap()
        }
    ...
    fun nbtToLua(nbt: Tag): Any? = when (nbt) {
        ...
        is CompoundTag -> {
            val map = nbt.keySet().associateWith { key -> nbtToLua(nbt.get(key)!!) }
            LuaNbtCompound(map)
        }
