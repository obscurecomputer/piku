...
/**
 * Renders [Component]s to their corresponding [computer.obscure.piku.mod.fabric.ui.components.UIComponent] implementations
 * to be rendered on the screen.
 */
class UIComponentRenderer(
    private val handlers: Map<Class<out Component>, computer.obscure.piku.mod.fabric.ui.components.UIComponent<out Component>>
) {
    fun draw(component: Component, context: GuiGraphics, instance: Minecraft) {
        val handler = handlers[component::class.java]
        if (handler != null) {
            @Suppress("UNCHECKED_CAST")
            (handler as computer.obscure.piku.mod.fabric.ui.components.UIComponent<Component>).draw(component, context, instance)
        } else {
